...
class Misc
    ...
    public static function setSound() /* Void */
        {
        shell_exec("export DISPLAY=:0; sudo qasmixer");
        }
    ...
    public static function makeBrowserHomePersistent(/* String */ $doPersist) /* Void */
        ...
        if ($doPersist=="on")
            ...
            @shell_exec("sudo killall chromium 2>/dev/null");
            @shell_exec("sudo /usr/bin/killall xinit >/dev/null 2>/dev/null");
            @shell_exec("sudo rm -fR /iwk/chromium-webkiosk-user.tar.gz 2>/dev/null");

            @shell_exec("sleep 2");
            @shell_exec("sudo rm -fR /home/pi/.config/chromium/chromium 2>/dev/null");
        ...
        // Relaunch X (xinit).
        @shell_exec("sudo /usr/bin/killall xinit >/dev/null 2>/dev/null"); // kill X.
        @shell_exec("sleep 2");
        @shell_exec("sudo /bin/sh /etc/rc.local >/dev/null 2>/dev/null &");
        }
    ...
    public static function changeResolution(/* Integer */ $w,/* Integer */ $h) /* Void */
        ...
        // Relaunch X (xinit).
        $killCommand = "sudo /usr/bin/killall xinit >/dev/null 2>/dev/null";
    ...
    public static function setStrickKioskMode(/* String */ $url,/* String */ $token,/* Integer */ $timeout,/* String */ $cache, /* String */ $keyboard, /* Integer */ $pageReload,/* String */ $haltAt,/* String */ $blankingTime,/* String */ $disableInput,/* String */ $proxy) /* Void */
        ...
        // Relaunch X (xinit).
        shell_exec("sudo /usr/bin/killall chromium-browser &>/dev/null && sudo /usr/bin/killall xinit &>/dev/null"); // www-data has root rights via sudo (no password specify required).
        shell_exec("sleep 3");
        shell_exec("sudo /bin/sh /etc/rc.local &>/dev/null &");
        }
